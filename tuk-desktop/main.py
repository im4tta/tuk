...
from PyQt6.QtCore import Qt, QTimer, QSettings, QSize, pyqtSignal, QByteArray, QBuffer
...
class ClipCard(QFrame):
    ...
    export_signal = pyqtSignal(object)
    ...
    def __init__(self, item, is_pinned=False):
        ...
        self.setFixedHeight(120 if self.is_image else 80)

        layout = QVBoxLayout(self)
        layout.setContentsMargins(14, 12, 14, 12)
        layout.setSpacing(6)

        # Preview row with thumbnail for images
        top = QHBoxLayout()
        ...
        if self.is_image:
            ...
            try:
                ...
                qimg = QImage.fromData(QByteArray(data.getvalue()))
                ...
                thumb_label.setStyleSheet("border-radius: 6px; border: 1px solid #45475a;")
            ...
            # Size info
            size_label = QLabel(f"{img.width if 'img' in dir() else '?'}×{img.height if 'img' in dir() else '?'}")
            size_label.setStyleSheet(f"color: {C['subtext0']}; font-size: 10pt; background: transparent;")
    ...
    def on_export(self):
        self.export_signal.emit(self.item)
...
class TukDesktopApp(QMainWindow):
    ...
    def refresh_captures(self):
        ...
        for it in self.history:
            ...
            card.export_signal.connect(self._export_item)
    ...
    def _copy_item(self, item):
        if item.is_image:
            # Copy image to system clipboard using Qt
            try:
                img = bytes_to_image(item.content)
                ...
                self.toast.show_message("📷 Image copied to clipboard!")
            except Exception as e:
    ...
    def _export_item(self, item):
        ...
        if file_path:
            try:
                img = bytes_to_image(item.content)
                img.save(file_path)
                self.toast.show_message(f"✅ Saved: {os.path.basename(file_path)}")
            except Exception as e:
                self.toast.show_message(f"Error saving: {e}")
