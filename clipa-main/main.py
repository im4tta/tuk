...
from PyQt6.QtGui import QFont, QIcon, QFontDatabase, QColor, QPixmap
from clipboard_manager import ClipboardMonitor, ClipboardItem, detect_type
from screenshot_capture import ScreenshotCapture, ImagePreviewCard
...
class ClipaApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        for key, label in [("editor", "✏️ កែសម្រួល"), ("history", "🕐 ប្រវត្តិ"),
                           ("pinned", "📌 ភ្ជាប់រហូត"), ("capture", "📷 ថតរូប"),
                           ("settings", "⚙️ ការកំណត់")]:
        ...
        self.stack.addWidget(self._capture_page())
    ...
    def _capture_page(self):
        """Screenshot capture page with history of captured images."""
        page = QWidget()
        v = QVBoxLayout(page)
        v.setContentsMargins(28, 28, 28, 28)
        v.setSpacing(18)
        ...
        # Capture button
        capture_btn = QPushButton("🎯 ថតរូបថតអេក្រង់")
        capture_btn.setObjectName("primaryBtn")
        capture_btn.clicked.connect(self.start_screenshot_capture)
        hdr.addWidget(capture_btn)
        v.addLayout(hdr)

        # Info label
        info = QLabel("ចុចលើប៊ូតុង 'ថតរូបថតអេក្រង់' ដើម្បីចាប់ផ្តើម បន្ទាប់មកជ្រើសរើសតំបន់ដែលចង់ថត")
        info.setStyleSheet(f"color: {C['subtext0']}; background: transparent; font-size: 10pt;")
        info.setWordWrap(True)
        v.addWidget(info)

        # Stats
        self.capture_stat = QLabel("0 រូបភាព")
    ...
    def switch_view(self, view):
        idx = {"editor": 0, "history": 1, "pinned": 2, "capture": 3, "settings": 4}[view]
    ...
    def start_screenshot_capture(self):
        """Initiate the screenshot capture overlay."""
        self.screenshot_capture.start_capture()

    def on_screenshot_captured(self, pixmap):
        """Handle captured screenshot."""
        if pixmap:
            self.captured_images.insert(0, {'pixmap': pixmap, 'timestamp': datetime.now()})
            self.refresh_captured_images()
            self.toast.show_message("បានថតរូប!")
            
            # Auto-copy to clipboard
            QApplication.clipboard().setPixmap(pixmap)

    def refresh_captured_images(self):
        ...
        for img_data in self.captured_images:
            card = ImagePreviewCard(img_data['pixmap'], img_data['timestamp'])
            card.copy_requested.connect(self.copy_image_to_clipboard)
            card.save_requested.connect(self.save_image)
            card.delete_requested.connect(lambda checked=False, d=img_data: self.delete_captured_image(d))
            self.capture_layout.insertWidget(self.capture_layout.count() - 1, card)

        # Update stats
        self.capture_stat.setText(f"{len(self.captured_images)} រូបភាព")

    def copy_image_to_clipboard(self, pixmap):
        """Copy image to system clipboard."""
        QApplication.clipboard().setPixmap(pixmap)
        self.toast.show_message("បានចម្លងរូបភាព!")
    ...
    def save_image(self, pixmap):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "រក្សាទុករូបភាព", default_name,
            "PNG Files (*.png);;JPEG Files (*.jpg);;All Files (*)"
        )
        
        if file_path:
            if pixmap.save(file_path):
                self.toast.show_message(f"បានរក្សាទុក: {os.path.basename(file_path)}")
            else:
                self.toast.show_message("មិនអាចរក្សាទុកបានទេ!")

    def delete_captured_image(self, img_data):
        """Delete a captured image."""
        if img_data in self.captured_images:
            self.captured_images.remove(img_data)
            self.refresh_captured_images()
            self.toast.show_message("បានលុបរូបភាព!")
